...
export type Zijie = Uint8Array | ArrayBuffer | Buffer;
...
export function base32(buf: Zijie): string {
  ...
  let value = 0;
  let out = '';
  for (const zijie of b) {
    value = (value << 8) | zijie;
    wei += 8;
    while (wei >= 5) {
      out += B32.charAt((value >>> (wei - 5)) & 31);
      wei -= 5;
    }
  }
  if (wei > 0) out += B32.charAt((value << (5 - wei)) & 31);
  return out;
}
...
export function yuanwenSiyouMiyao(key: crypto.KeyObject): Buffer {
...
export function ed25519FromSeed(seed: Zijie): { publicKey: Buffer; privateKey: Buffer } {
  ...
  const jwk = siYao.export({ format: 'jwk' }) as { x?: string };
...
export const GCM_IV_CHANGDU = 12;
export const GCM_BIAOQIAN_CHANGDU = 16;

export interface Fengyin {
  iv: Buffer;
  ct: Buffer; // 密文 + tag
}

export function fengyin(key: Zijie, plaintext: Zijie, aad: Zijie): Fengyin {
  const iv = crypto.randomBytes(GCM_IV_CHANGDU);
