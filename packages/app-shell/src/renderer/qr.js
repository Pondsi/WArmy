 ...
 *
 * 两份实现（app.js 的 qrSvg 与这里的 qrSvg）的重复是**故意的**：
...
function loadClassicEncoder() {
  ...
  // 链接里可能有非 ASCII（别名/名字）——上游默认是 Latin-1 式映射，显式换 UTF-8（与 app.js 一致）
...
export function qrMatrix(text, ecc = QR_ECC) {
  const qr = qrcode(0, ecc); // typeNumber 0 = 版本自适应
  qr.addData(String(text == null ? '' : text), 'Byte');
  qr.make();
 ...
 * @param {string} label aria-label（i18n 文案；验收比对时会被规范化掉，另行与语言包逐字比）
 */
export function qrSvg(text, size = 168, ecc = QR_ECC, label = '') {
  ...
  return '<svg xmlns="http://www.w3.org/2000/svg" width="' + size + '" height="' + size +
    '" viewBox="0 0 ' + size + ' ' + size + '" role="img" aria-label="' + label + '"' + attrs + '>' +
