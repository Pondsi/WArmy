...
async function attachTo(pred, label, tries = 90) {
      ...
      if (page) {
        ...
        return {
          ...
          evaluate: async (expr) => {
            ...
            if (r.exceptionDetails) throw new Error(label + ': ' + String(r.exceptionDetails.exception?.description || r.exceptionDetails.text).slice(0, 200));
  ...
  throw new Error('等不到 CDP target: ' + label);
}
...
const VIEW = `(function(){
  ...
  return {
    title: txt('#chat-title').trim(),
    bubbles: Array.from(document.querySelectorAll('#messages .bubble')).map((b) => b.textContent.trim()),
    hasInput: !!q('#input'), hasSend: !!q('#btn-send'),
    right: txt('#project-state-box').replace(/\\s+/g, ' ').trim().slice(0, 40),
  };
})()`;
...
try {
  ...
  const made = await main.evaluate(`(async function(){
    const out = [];
    for (const [gid, name] of [['${g1}','${n1}'], ['${g2}','${n2}']]) {
      const r = await window.warmy.groupCreate({ groupId: gid, name, type: 'internal', directedMode: false, devEnv: 'host' });
      out.push(!!(r && r.ok));
    }
  ...
  await sleep(2000);
  const pick = async (name) => main.evaluate(`(function(){
    const rows = Array.from(document.querySelectorAll('#list-body .list-item'));
    const hit = rows.find((r) => ((r.querySelector('.name') || {}).textContent || '').trim() === ${JSON.stringify(name)});
    if (hit) hit.click();
    return { hit: !!hit, title: (document.querySelector('#chat-title') || {}).textContent || '' };
  })()`);
  ...
  // ① 发一条
  const t1 = '主窗-' + stamp;
  await main.evaluate(`(function(){ const i=document.querySelector('#input'), b=document.querySelector('#btn-send'); i.value=${JSON.stringify(t1)}; i.dispatchEvent(new Event('input',{bubbles:true})); b.click(); return true; })()`);
  await sleep(4000);
  ...
  check('独立窗与主界面：**消息集合一致**（同一套数据）', JSON.stringify(s1.bubbles) === JSON.stringify(m1.bubbles), { sub: s1.bubbles, main: m1.bubbles });
  check('独立窗与主界面：右侧一致', s1.right === m1.right, { sub: s1.right, main: m1.right });
  check('独立窗：有输入区（可在新窗口继续聊）', s1.hasInput && s1.hasSend);

  // ③ 在独立窗发一条 → 主界面同一会话应出现
  const t2 = '新窗-' + stamp;
  await sub1.evaluate(`(function(){ const i=document.querySelector('#input'), b=document.querySelector('#btn-send'); i.value=${JSON.stringify(t2)}; i.dispatchEvent(new Event('input',{bubbles:true})); b.click(); return true; })()`);
  await sleep(4500);
  ...
  check('第二个会话的右栏不是第一个会话的', !m3.right.includes(n1), { right: m3.right });
