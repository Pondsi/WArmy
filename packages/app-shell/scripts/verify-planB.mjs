...
check('listWidth persisted in bindResizer', /persistKey:\s*'listWidth'/.test(app));
check('panelWidth persisted', /persistKey:\s*'panelWidth'/.test(app));
check('boot applies listWidth', app.includes('s.settings.listWidth'));
check('boot applies panelWidth', app.includes('s.settings.panelWidth'));
...
check('contextTooSmall i18n key', app.includes('chat.contextTooSmall') || main.includes('chat.contextTooSmall'));
...
check('genSessionSummary in app.js', /genSessionSummary/.test(app));
check('auto-summary toggle', /auto-summary-toggle/.test(idx) || /auto-summary-toggle/.test(app));
check('autoSummary default true', /autoSummary:\s*true/.test(ss));
check('summary jump by recordId', /searchMessages/.test(app) && /recordId/.test(app));
check('renderPanelSummary exists', /renderPanelSummary/.test(app));

// ── 5. logo 无背景 ──
check('renderer.css tb-logo transparent', /\.tb-logo\s*\{[^}]*background:\s*transparent/.test(rcss));
check('renderer.css logo-glyph transparent', /\.logo-glyph\s*\{[^}]*background:\s*transparent/.test(rcss));
check('app.css logo-glyph transparent', /\.logo-glyph\s*\{[^}]*background:\s*transparent/.test(acss));
check('no gradient on tb-logo', !/\.tb-logo\s*\{[^}]*linear-gradient/.test(rcss));

// ── 6. 成员栏按类型 ──
check('members only internal/external group', /panel-members-block.*kind === 'internal'/.test(app) || /set\('panel-members-block'/.test(app));

// ── 7. ctx popover 跟随按钮 ──
check('ctx popover uses getBoundingClientRect', /getBoundingClientRect/.test(app));
check('ctx popover not fixed bottom', !/ctx-popover\s*\{[^}]*bottom:\s*150px/.test(rcss));
