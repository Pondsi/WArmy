 ...
 *   2. 不丢细节：每个 elided range 都能 retrieve({recordId}) / retrieve({seq}) / recall(tiShi)
...
const { xuanranYoujieShitu, DEFAULT_CONTEXT_BUDGET_CHARS, MAX_RANGE_RECORD_IDS } = await import(pathToFileURL(distRenderer).href);
...
function makeLog(count, charsPerEntry) {
  const ti = '牛马值班日志：项目 WArmy 的上下文渲染条目。'.repeat(Math.ceil(charsPerEntry / 22));
  const entries = [];
  for (let i = 0; i < count; i++) {
    entries.push({
      seq: i + 1,
      role: i % 2 === 0 ? 'user' : 'assistant',
      content: `#${i + 1} ${ti}`.slice(0, charsPerEntry),
      recordId: `m-${i + 1}`,
      ts: 1700000000000 + i,
    });
  }
...
const viewChars = (v) => v.xiaoXiJi.reduce((s, m) => s + m.content.length, 0);
const viewUtf8 = (v) => v.xiaoXiJi.reduce((s, m) => s + Buffer.byteLength(m.content, 'utf8'), 0);
const variance = (xs) => {
...
for (const n of SIZES) {
  ...
  const view = xuanranYoujieShitu(log, { budgetChars: BUDGET, recallHint: '值班者状态机' });
...
for (const r of rows) {
  console.log(`  ${pad(r.n, 10)} ${pad(r.logBytes, 15)} ${pad(r.logUtf8, 15)} ${pad(r.viewBytes, 11)} ${pad(r.viewUtf8, 10)} ${pad(r.xiaoXi, 8)} ${pad(r.pointers, 6)} ${pad(r.elided, 8)} ${pad(r.ms, 8)}`);
}
...
// ══════════════════════════════════════════════════════════════
// 2. 不丢细节（真 JiyiCangFuwu：写入 → 渲染 → 用指针取回）
// ══════════════════════════════════════════════════════════════
...
for (let i = 0; i < N; i++) {
  ...
  const ti = `真记录 ${i}｜值班者状态机与上下文有界渲染器验证\n` + `payload-${i}-` + 'X'.repeat(200) + `-end-${i}`;
  const rec = mem.append({ id, sessionId: 's-verify', kind: 'message', ti }, 'duty');
  bodies.set(id, ti);
  log.push({ seq: rec.seq, role: i % 2 === 0 ? 'user' : 'assistant', content: ti, recordId: id, ts: Date.now() });
}
const memView = xuanranYoujieShitu(log, { budgetChars: 1200, recallHint: '值班者状态机' });
...
// (c) 语义线索：recall(tiShi) → 卡片 anchor → retrieve 逐字节一致
...
const pointerText = memView.xiaoXiJi.map((m) => m.content).join('\n');
...
const headTailInView = memView.xiaoXiJi.length - 1; // 减去指针那条
check(
...
//     不写死具体条数 —— 采样策略变化时这些断言仍然成立。
{
  ...
  const a = xuanranYoujieShitu(makeLog(10000, 512), { budgetChars: BUDGET, recallHint: 'tiShi' });
  const t0 = Date.now();
  const b = xuanranYoujieShitu(makeLog(100000, 512), { budgetChars: BUDGET, recallHint: 'tiShi' });
...
console.log('\n[3] 头尾保真：keepHead / keepTail 条逐字节等于原文');
{
  ...
  const view = xuanranYoujieShitu(big, { budgetChars: BUDGET, keepHead: 1, keepTail: 8, recallHint: 'tiShi' });
  const headOk = view.xiaoXiJi[0].content === big[0].content;
  // 尾部条数由渲染器按预算决定（要点要占预留份额，所以可能少于请求的 keepTail=8）；
  // 断言的是「保留下来的尾段逐字节等于原文」，不是「必须有 8 条」。
  const ptrIdx = view.xiaoXiJi.findIndex((m) => m.role === 'system');
  const tailMsgs = ptrIdx >= 0 ? view.xiaoXiJi.slice(ptrIdx + 1) : view.xiaoXiJi.slice(-8);
  const tailOk =
    tailMsgs.length > 0 &&
    tailMsgs.every((m, i) => m.content === big[big.length - tailMsgs.length + i].content);
  check('keepHead=1 逐字节等于原文', headOk && view.xiaoXiJi[0].role === big[0].role);
  ...
  check(
    ...
    ptrIdx === view.xiaoXiJi.length - 1 - tailMsgs.length && view.xiaoXiJi[ptrIdx].content.includes('已省略')
  );
  // 中间某条被省略的原文不在视图里（真的被挤出去了）
  const mid = big[Math.floor(big.length / 2)].content;
  check('被省略的中间条目不在视图里（确实发生了裁剪）', !view.xiaoXiJi.some((m) => m.content === mid));
}
...
console.log('\n[4] 退化预算与脏输入');
{
  ...
  let view200 = null;
  try {
    view200 = xuanranYoujieShitu(log, { budgetChars: 200, recallHint: '值班者状态机' });
  } catch (e) {
    threw = String(e);
  }
  ...
  check('budgetChars=200 仍有消息且有指针', view200 && view200.xiaoXiJi.length >= 1 && view200.stats.pointers >= 1, view200 && {
    xiaoXi: view200.xiaoXiJi.length,
    pointers: view200.stats.pointers,
    elided: view200.elided.reduce((s, r) => s + r.count, 0),
  });
  const p200 = view200.xiaoXiJi.map((m) => m.content).join('\n');
  ...
  for (const b of budgets) {
    try {
      const v = xuanranYoujieShitu(log, { budgetChars: b });
  ...
  for (const d of dirty) {
    try {
      xuanranYoujieShitu(d, { budgetChars: 200 });
      xuanranYoujieShitu(d, { budgetChars: 200, compress: () => { throw new Error('boom'); } });
      xuanranYoujieShitu(d, { budgetChars: 200, compress: () => 12345 });
      xuanranYoujieShitu(d, null);
    } catch (e) {
      dirtyThrew = `${JSON.stringify(d)?.slice(0, 40)}: ${e}`;
    }
  }
  check('脏输入 + 压缩器抛错都不抛错', !dirtyThrew, dirtyThrew || 'ok');
  const cthrow = xuanranYoujieShitu(makeLog(50, 300), { budgetChars: 1000, compress: () => { throw new Error('boom'); } });
...
const mockBase = `http://127.0.0.1:${mockPort}/v1`;
{
  ...
  router.createGroup({
    groupId: 'g-ctx', ming: '有界渲染验证群', type: 'internal', dutyInstanceId: null,
    directedMode: false, members: [], permissions: DEFAULT_PERMISSIONS, checkpointLimit: 50,
  });
  router.join('g-ctx', { id: 'duty-1', ming: '值班者', local: true, dutyEligible: true, status: 'idle' });
  ...
  const deps = {
    ...
    history: new Map(),
    listInstances: () => [{ id: 'duty-1', ming: '值班者', status: 'idle', dutyEligible: true }],
    logOf: () => dutyLog,
...
if (!SKIP_ELECTRON) {
  ...
  // prepareMemoryRuntime 的第二条候选路径挂在 yingYong.getAppPath() 下（Electron 启动单文件时 = dist/）
  ...
  if (cdp) {
    ...
    if (ids.length && Array.isArray(memRecall?.cards)) {
      ...
      const longOnes = records.filter((r) => r.seq >= lo && r.seq <= hi && typeof r.ti === 'string' && r.ti.length >= 100);
      const midRec = longOnes[Math.floor(longOnes.length / 2)];
      const inView = midRec ? ptrAll.includes(midRec.ti) : null;
      check('被省略的长正文记录不在视图里，但日志里逐字节可取回', !!midRec && midRec.ti.length >= 100 && inView === false, {
        range: [lo, hi],
        pickedSeq: midRec?.seq,
        bodyChars: midRec?.ti?.length ?? null,
        inView,
