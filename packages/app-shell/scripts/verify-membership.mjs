...
for (const [label, id] of [['creator', creator], ['bob', memberB], ['carol', memberC], ['evil', attacker]]) {
  const u = id.store.unlock(PASS);
  if (!u.ok) {
    console.error(`夹具身份 ${label} 解锁失败：`, u);
    process.exit(1);
  }
}
...
check('canonicalMembershipJson 键序无关（同值同串）', canonicalMembershipJson({ b: 1, a: 2 }) === canonicalMembershipJson({ a: 2, b: 1 }), canonicalMembershipJson({ b: 1, a: 2 }));
check('canonicalMembershipJson 丢 undefined 字段', canonicalMembershipJson({ a: 1, b: undefined }) === canonicalMembershipJson({ a: 1 }));
check('未签名证书（issuerSignature 为空）必须被拒', verifyMemberCertificate(buildMemberCertificate({
...
const described = describeMemberCertificate(bobCert);
...
for (const [label, mutate] of tamperCases) {
  ...
  if (label.includes('未知字段')) {
    check(`篡改「${label}」→ 仍通过（载荷只含列出的字段，这是刻意的）`, r.ok === true, r.code);
  } else {
    check(`篡改「${label}」→ 验签/校验失败`, r.ok === false, r.code);
  }
}
...
check('把 issuerSignature 换成"用别的私钥对同一载荷重签的合法签名" → 仍失败（验签用的是 issuerPublicKey）', (() => {
  const evilKeys = generateEd25519();
...
const shortCertBase = buildMemberCertificate(
  {
...
check('findRevocationEntry 命中成员指纹', !!findRevocationEntry(creatorMembership.revocation(GROUP), { memberFingerprint: infoC.fingerprint }));
...
check('吊销条目 revokedAt 被改成"一年后"依然吊销（不看时间戳）', (() => {
  ...
  return isRevoked(tampered, { certId: 'mc-carol-1' }) === true && !!findRevocationEntry(tampered, { memberFingerprint: infoC.fingerprint });
})());
...
const v2 = await signRevocationList(
  buildRevocationList({ groupId: GROUP, listVersion: 2, entries: [...curList.entries], issuerFingerprint: infoCreator.fingerprint, issuerPublicKey: infoCreator.publicKey }),
  (b) => signerCreator.sign(b)
);
const applied2 = creatorMembership.applyRevocationList(GROUP, v2);
...
const sameVersionDifferent = await signRevocationList(
  buildRevocationList({ groupId: GROUP, listVersion: 2, entries: [], issuerFingerprint: infoCreator.fingerprint, issuerPublicKey: infoCreator.publicKey, issuedAt: v2.issuedAt + 5000 }),
  (b) => signerCreator.sign(b)
);
const replayRes = creatorMembership.applyRevocationList(GROUP, sameVersionDifferent);
...
check('同版本(v2)且内容相同 → 幂等接受（changed=false）', (() => {
  const r = creatorMembership.applyRevocationList(GROUP, v2);
  return r.ok === true && r.changed === false;
})());
const v3Dropped = await signRevocationList(
  buildRevocationList({ groupId: GROUP, listVersion: 3, entries: [], issuerFingerprint: infoCreator.fingerprint, issuerPublicKey: infoCreator.publicKey }),
  (b) => signerCreator.sign(b)
);
...
const evilList = await signRevocationList(
  buildRevocationList({ groupId: GROUP, listVersion: 9, entries: [{ certId: 'mc-bob-1', memberFingerprint: infoB.fingerprint, reason: 'admin', revokedAt: nowMs }], issuerFingerprint: infoEvil.fingerprint, issuerPublicKey: infoEvil.publicKey }),
...
check('把攻击者的列表塞进本机 → 被拒且版本不变', (() => {
  const r = creatorMembership.applyRevocationList(GROUP, evilList);
  return r.ok === false && creatorMembership.revocation(GROUP).listVersion === 2;
})(), creatorMembership.applyRevocationList(GROUP, evilList).code);
...
check('applyInboundRevocationUpdate：本机不知道群主时如实拒（unknown-issuer，不猜）', (() => {
  const fresh = new MembershipStore(path.join(tmpRoot, 'unknown-issuer.json'), { fingerprintOf: fingerprintFromPublicKey });
...
check('chainRootCertId 指向链根（旧 certId）', chainRootCertId(creatorMembership.listCertificates(GROUP), 'mc-bob-2') === CREATOR_CERT_ID, chainRootCertId(creatorMembership.listCertificates(GROUP), 'mc-bob-2'));
...
check('carol 的证书链根是她自己（无 supersedes）', chainRootCertId(creatorMembership.listCertificates(GROUP), 'mc-carol-1') === 'mc-carol-1');
...
check('requireCertificate=true 但持有效证书 → 放行', (() => {
  const m = new MembershipStore(path.join(tmpRoot, 'plain-membership.json'), { fingerprintOf: fingerprintFromPublicKey, clockSkewMs: DEFAULT_CLOCK_SKEW_MS });
...
check('证书记录优先于 TOFU：被吊销者即使在本机联系人表里也被拒', (() => {
  const m = new MembershipStore(path.join(tmpRoot, 'plain-membership2.json'), { fingerprintOf: fingerprintFromPublicKey, clockSkewMs: DEFAULT_CLOCK_SKEW_MS });
  ...
  const list = creatorMembership.revocation(GROUP);
  m.applyRevocationList(GROUP, list, { expectIssuerFingerprint: infoCreator.fingerprint });
...
check('过期证书也要落盘（stored=true）→ 名册因此明确拒，而不是退回 TOFU', (() => {
  const m = new MembershipStore(path.join(tmpRoot, 'expired-store.json'), { fingerprintOf: fingerprintFromPublicKey, clockSkewMs: DEFAULT_CLOCK_SKEW_MS, now: () => nowMs });
  ...
  const m = new MembershipStore(path.join(tmpRoot, 'expired-store-probe.json'), { fingerprintOf: fingerprintFromPublicKey, clockSkewMs: DEFAULT_CLOCK_SKEW_MS, now: () => nowMs });
  const expired = independentSignCert(
    buildMemberCertificate(
...
check('名册判定在"证书有效"时给出 basis=certificate', (() => {
  const m = new MembershipStore(path.join(tmpRoot, 'plain-membership3.json'), { fingerprintOf: fingerprintFromPublicKey, clockSkewMs: DEFAULT_CLOCK_SKEW_MS });
...
const legacy = {
  version: 1,
  groups: [{ groupId: 'g-legacy', name: '旧群', type: 'internal', directedMode: false, dutyInstanceId: null, createdAt: 1, updatedAt: 1, origin: 'ipc' }],
  members: {
    'g-legacy': [
      { id: 'm-1', name: '老王', role: 'member', joinedAt: 1, source: 'invite' },
      { id: 'inst:inst-9', name: '牛马九号', role: 'member', joinedAt: 2, source: 'instance', instanceId: 'inst-9' },
    ],
  },
};
fs.writeFileSync(groupsFile, JSON.stringify(legacy, null, 2), 'utf8');
const legacyRaw = fs.readFileSync(groupsFile, 'utf8');
const gs = new GroupStore(groupsFile);
...
const inviteWithFp = gs.addMemberWithFingerprint(
  'g-legacy',
  { name: '小张', role: 'member', source: 'invite', fingerprint: infoB.fingerprint },
...
const audits2 = [];
const inviteNoFp = gs.addMemberWithFingerprint('g-legacy', { name: '无名', role: 'member', source: 'invite' }, { onAudit: (op, detail) => audits2.push({ op, detail }) });
...
const instMember = gs.addMemberWithFingerprint(
  'g-legacy',
  { name: '牛马十号', role: 'member', source: 'instance', instanceId: 'inst-10', fingerprint: infoC.fingerprint },
...
const audits4 = [];
gs.addMemberWithFingerprint('g-legacy', { name: '迁移员', role: 'member', source: 'migrated' }, { onAudit: (op, detail) => audits4.push({ op, detail }) });
...
check('addMember（底层）也接受 fingerprint 参数', (() => {
  const r = gs.addMember('g-legacy', { name: '直接加', role: 'member', source: 'invite', fingerprint: infoC.fingerprint });
  return r.ok === true && r.members.find((m) => m.name === '直接加')?.fingerprint === infoC.fingerprint;
})());
const gs2 = new GroupStore(groupsFile);
const up = gs2.upsertGroup({ groupId: 'g-legacy', name: '旧群', type: 'internal', creatorFingerprint: infoCreator.fingerprint });
...
check('再次 upsert 不覆盖已知的 creatorFingerprint', (() => {
  const r = gs2.upsertGroup({ groupId: 'g-legacy', name: '旧群2', type: 'internal', creatorFingerprint: infoEvil.fingerprint });
  return r.group?.creatorFingerprint === infoCreator.fingerprint;
})());
check('不给 creatorFingerprint 时保持缺失（不编造）', (() => {
  const r = gs2.upsertGroup({ groupId: 'g-new', name: '新群', type: 'external' });
...
const scopeGroups = new GroupStore(path.join(tmpRoot, 'scope-groups.json'));
scopeGroups.upsertGroup({ groupId: 'g-in', name: '项目推进群', type: 'internal', creatorFingerprint: infoCreator.fingerprint });
scopeGroups.upsertGroup({ groupId: 'g-ext', name: '外部队', type: 'external', creatorFingerprint: infoCreator.fingerprint });
scopeGroups.addMemberWithFingerprint('g-in', { name: 'bob', role: 'member', source: 'invite', fingerprint: bobInfoAfter.fingerprint });
scopeGroups.addMemberWithFingerprint('g-ext', { name: 'bob', role: 'member', source: 'invite', fingerprint: bobInfoAfter.fingerprint });
scopeGroups.addMemberWithFingerprint('g-ext', { name: '本地牛马', role: 'member', source: 'instance', instanceId: 'inst-1' });
...
check('命中群列表非空且带成员 id/name（可追责）', scoped.matchedGroups.length === 2 && scoped.matchedGroups.every((g) => !!g.memberId), scoped.matchedGroups);
...
const liveness = new ConnectionLiveness({ now: () => livenessClock, offlineFailures: 2, offlineAfterMs: 30_000 });
...
const presenceMembers = [
  { id: 'inst-1', name: '牛马一号', source: 'instance', instanceId: 'inst-1' },
...
const reopened = new MembershipStore(membershipFileFor(creator.store), {
  fingerprintOf: fingerprintFromPublicKey,
  clockSkewMs: DEFAULT_CLOCK_SKEW_MS,
});
check('重开（模拟重启）后证书仍在', reopened.listCertificates(GROUP).length === creatorMembership.listCertificates(GROUP).length, {
  reopened: reopened.listCertificates(GROUP).length,
...
reopened.applyRevocationList(GROUP, vNext, { expectIssuerFingerprint: infoCreator.fingerprint });
...
const brokenStore = new MembershipStore(brokenFile, { fingerprintOf: fingerprintFromPublicKey });
...
const junkStore = new MembershipStore(junkFile, { fingerprintOf: fingerprintFromPublicKey });
...
check('membershipSnapshot 对"已过期"的证书给出 valid=false + code=expired', (() => {
  const m = new MembershipStore(path.join(tmpRoot, 'snap-expired.json'), { fingerprintOf: fingerprintFromPublicKey, clockSkewMs: DEFAULT_CLOCK_SKEW_MS });
  const expired = independentSignCert(
    buildMemberCertificate(
...
check('跨群一致性：吊销与"有效证书"并存时**与写入顺序无关**，一律拒（fail-closed）', (() => {
  const mkOrderedStore = async (file, revocationFirst) => {
    const m = new MembershipStore(file, { fingerprintOf: fingerprintFromPublicKey, clockSkewMs: DEFAULT_CLOCK_SKEW_MS });
    ...
    const applied = m.applyRevocationList(revokeGroup, revocationList, { expectIssuerFingerprint: infoCreator.fingerprint });
...

{
  ...
  const proto = MembershipStore.prototype;
  const original = proto.runExclusive;
  const bypassFile = path.join(tmpRoot, 'concurrent-bypass.json');
  const bypassStore = new MembershipStore(bypassFile, { fingerprintOf: fingerprintFromPublicKey, clockSkewMs: DEFAULT_CLOCK_SKEW_MS });
