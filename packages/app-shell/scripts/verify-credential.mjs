 ...
 */
import { generateCredential, formatCredential, normalizeCredential, isValidCredential, keyPairFromCredential, CREDENTIAL_ALPHABET, CREDENTIAL_LENGTH } from '../dist/credential.js';
...
check('凭证长度 = 45', norm.length === CREDENTIAL_LENGTH, norm.length);
check('字符集为 56 符号（数字+大小写字母，去掉 I/O/Z）', CREDENTIAL_ALPHABET.length === 56, CREDENTIAL_ALPHABET.length);
...
const bits = norm.length * Math.log2(CREDENTIAL_ALPHABET.length);
...
const c = keyPairFromCredential(generateCredential());
