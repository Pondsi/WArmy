...
if (!j.includes('yiGuiDangHe')) {
  ...
  if (j.includes(about)) {
    ...
    console.log('about anchor missing in yingYong.js');
  }
}
...
const oldKb = "    $('zhiShiKuShuChu').textContent =\n      (r?.entities || []).map((e) => e.name).join(', ') +\n      ' | ' +\n      (r?.events || []).map((e) => e.biaoTi).join(', ');";
if (j.includes(oldKb)) {
  j = j.replace(oldKb, `    const det = await window.warmy.kbDetail(q).catch(() => null);
    $('zhiShiKuShuChu').innerHTML =
      ...
      '<div>' + escapeHtml((r?.events || []).map((e) => e.biaoTi).join(' | ') || '—') + '</div>' +
      (det?.entities?.length
  ...
  const i = j.indexOf('zhiShiKuShuChu');
  if (i >= 0) console.log('zhiShiKuShuChu context:', JSON.stringify(j.slice(i - 80, i + 200)));
}

fs.writeFileSync(base + 'yingYong.js', j);
console.log('final:', j.includes('refreshArchived'), j.includes('yiGuiDangHe'), j.includes('kbDetail'));
