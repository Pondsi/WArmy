...
// 修复2：xiaoXiJi 容器加 content-visibility 隔离重绘
c = c.replace(
  '.xiaoXiJi { flex: 1; overflow: auto; padding: var(--sp-4); }',
  `.xiaoXiJi {
...
console.log('xiaoXiJi content-visibility');

// 修复3：#shuRu 加 contain 隔离，避免打字触发外部重排
c = c.replace(
  `#shuRu {
...
console.log('shuRu contain');

// 修复4：liaoTianLan 加 contain
c = c.replace(
  `#liaoTianLan {
...
console.log('liaoTianLan contain');

// 修复5：xiaoXi 动画改为 opacity-only（避免 layout）
c = c.replace(
  '.xiaoXi { display: flex; margin-bottom: 14px; gap: 10px; animation: fade-in 0.15s ease; }\n@keyframes fade-in { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: none; } }',
  '.xiaoXi { display: flex; margin-bottom: 14px; gap: 10px; animation: msg-in 0.12s ease; }\n@keyframes msg-in { from { opacity: 0; } to { opacity: 1; } }'
);
console.log('xiaoXi animation opacity-only');

// 修复6：jinDuTiao transition 去掉（每 tick 刷新会触发）
c = c.replace(
  '.jinDuTiao { height: 100%; background: var(--accent); border-radius: 99px; transition: width 0.25s; }',
  '.jinDuTiao { height: 100%; background: var(--accent); border-radius: 99px; }'
);
console.log('jinDu transition removed');

// 修复7：所有 ceLanTiaoMu / anNiuTuBiao 的 transition 去掉 transform
c = c.replace(
