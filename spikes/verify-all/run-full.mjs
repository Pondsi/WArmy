...
function check(ming, cond, detail) {
  if (cond) {
    ...
    fail++;
    fails.push({ ming, detail });
    console.log(`FAIL  ${ming}`, detail ?? '');
  }
}
...
check('displayName zh ming', zh['yingYong.zhName'] === '无限牛马');
check('displayName en ming', en['yingYong.enName'] === 'WArmy');
for (const k of ['nav.settings', 'chat.faSong', 'dashboard.biaoTi', 'wo.username', 'settings.providers', 'chat.stopAll']) {
...
check('duiHuaKuangGen in html', html.includes('id="duiHuaKuangGen"'));
check('username display', appJs.includes('usernameShuRu') || appJs.includes('usernameDisplay'));
check('username shuRu', appJs.includes('id="pMing"'));
check('light ceLan', appCss.includes('--rail-bg: #ebebeb') || appCss.includes('--rail-bg:#ebebeb'));
check('dark ceLan', appCss.includes('[data-theme="dark"]'));
check('ceLan jiHuo accent', appCss.includes('--rail-active: var(--accent)'));
check('resizer list', html.includes('lanTiaoZhengTiao'));
check('resizer panel', html.includes('mianBanTiaoZhengTiao'));
check('voice btn', html.includes('anNiuYuYin'));
check('attach btn', html.includes('anNiuAttach'));
check('menu removed', mainTs.includes('Menu.setApplicationMenu(null)'));
check('group create ipc', mainTs.includes('warmy:qunChuangJian'));
check('board ipc', mainTs.includes('warmy:kanbanRenwuJi'));
check('list models ipc', mainTs.includes('warmy:lieBiaoMoXingJi'));
check('theme ipc', mainTs.includes('warmy:sheZhiZhuTiLaiYuan'));
...
const { congYuSheChuangJian, GONGYING_YUSHE, guiFanYongLiang } = await import(
...
router.createGroup({
  groupId: 'g1', ming: 'G', type: 'internal', dutyInstanceId: null,
  directedMode: false, members: [], permissions: DEFAULT_PERMISSIONS, checkpointLimit: 50,
});
router.join('g1', { id: 'a', ming: 'A', local: true, dutyEligible: true, status: 'idle' });
router.join('g1', { id: 'r', ming: 'R', local: false, dutyEligible: true, status: 'idle' });
...
check('chat-send ipc', mainTs.includes('warmy:liaoTianFaSong'));
...
check('save-voice ipc', mainTs.includes('warmy:baoCunYuYin'));
check('metrics ipc', mainTs.includes('warmy:zhiBiaoJiZhaiYao'));
check('nodes ipc', mainTs.includes('warmy:jieDianJiLieBiao'));
check('profile login ipc', mainTs.includes('warmy:profileLogin'));
check('settings persist ipc', mainTs.includes('warmy:peiZhiBaoCun'));
...
check('executor ipc', mainTs.includes('warmy:zhiXingQiYunXing'));
check('assets ipc', mainTs.includes('warmy:ziChanJiJianSuo'));
check('kb-from-chat ipc', mainTs.includes('warmy:zhiShiKuCongLiaoTian'));
...
check('orchestrate ipc', mainTs.includes('warmy:qunXieTiao'));
check('approval ipc', mainTs.includes('warmy:qingQiuPiZhun'));
check('cost ipc', mainTs.includes('warmy:chengBenZhaiYao'));
check('status ka', fs.readFileSync(path.join(root, 'packages/app-shell/src/orchestrator.ts'), 'utf8').includes('buildStatusCard'));
check('renderer approval modal', appJs.includes('showApprovalDialog'));
check('renderer cost', appJs.includes('costSummary'));
check('renderer kb save', html.includes('anNiuZhiShiKuBaoCun'));
...
check('executors-status ipc', mainTs.includes('warmy:zhiXingQiJiZhuangTai'));
check('asr ipc', mainTs.includes('warmy:asrZhuanXie'));
check('state-save ipc', mainTs.includes('warmy:taiBaoCun'));
check('renderer exec panel', html.includes('zhiXingHe'));
...
check('model add/del', appJs.includes('iDelMoXing') && appJs.includes('iTianJiaMoXing'));
check('open-chat-window ipc', mainTs.includes('warmy:daKaiLiaoTianChuangKou'));
check('hotkey ipc', mainTs.includes('warmy:zhuCeKuaiJieJian'));
check('tray ipc', mainTs.includes('warmy:tuoPanChuShi'));
check('export ipc', mainTs.includes('warmy:daoChuHuiHua'));
check('auto-update ipc', mainTs.includes('warmy:ziDongGengXinJianCha'));
check('checkpoint mtime', fs.readFileSync(path.join(root, 'packages/app-shell/src/checkpoint.ts'), 'utf8').includes('mtimeMs'));
check('renderer daKai window', appJs.includes('btn-open-win'));
...
check('group-members ipc', mainTs.includes('warmy:qunChengYuanJi'));
check('kanbanHuiHua ipc', mainTs.includes('warmy:kanbanHuiHua'));
check('ccr-tool ipc', mainTs.includes('warmy:ccrGongJuShuChu'));
check('kb-detail ipc', mainTs.includes('warmy:zhiShiKuXiangQing'));
check('last-error ipc', mainTs.includes('warmy:zuiHouCuoWu'));
check('setup ipc', mainTs.includes('warmy:chuShiSheZhiTai'));
...
check('search-messages ipc', mainTs.includes('warmy:souSuoXiaoXiJi'));
check('plugin-install ipc', mainTs.includes('warmy:chaJianAnZhuang'));
check('archived ipc', mainTs.includes('warmy:yiGuiDangLieBiao'));
check('renderer chat search', appJs.includes('btn-chat-search'));
check('renderer directed', appJs.includes('btn-directed'));
check('html directed', html.includes('caiDanTuBiaoDingXiang'));
...
check('html cost dash', html.includes('chengBenDash') && html.includes('anNiuChengBenCsv'));
check('html archived', appJs.includes('yiGuiDangHe'));
...
check('audit ipc', mainTs.includes('warmy:shenJiRiZhi'));
check('safeStorage ipc', mainTs.includes('warmy:anQuanMiYaoBaoCun'));
check('archive ipc', mainTs.includes('warmy:guiDangWaiBu'));
check('cleanup ipc', mainTs.includes('warmy:qingLiYunXing'));
check('role-models ipc', mainTs.includes('warmy:jueSeMoXingJiSheZhi'));
check('dissolve ipc', mainTs.includes('warmy:qunJieSan'));
check('export-allowlist ipc', mainTs.includes('warmy:daoChuYunXuMingDan'));
...
check('dsh ipc', mainTs.includes('warmy:paiShengdshShiLi'));
check('email ipc', mainTs.includes('warmy:youJianDuiLie'));
...
check('smtp verify ipc', mainTs.includes('warmy:smtpYanZheng'));
check('lan start ipc', mainTs.includes('warmy:neiWangQiDong'));
check('lan dual smoke', mainTs.includes('warmy:neiWangShuangJiMaoYan'));
check('smtp not hardcoded', !mainTs.includes('smtp.qq.com') && !mainTs.includes('@gmail.com'));
check('renderer smtp add btn', appJs.includes('anNiusmtpTianJia') && appJs.includes('smtpAdd'));
// 内网同步 / 多节点组网：产品负责人**明确要求**把这两个设置块从 UI 移除（功能由下方
// 「组网设置」卡片 id="wangLuoKa" 承接）；但底层 IPC 通道 warmy:lan-* / warmy:mesh-*
// 仍是**产品契约**——删掉就是破坏契约变更。所以拆成一对，比原来单看一个 id 更严：
//  ① 旧入口在渲染层**不存在**了（原区块的全部 DOM id 都查一遍），且替代入口（wangLuoKa）必须在
//     ——否则"整段删掉什么都不过关"也能骗过纯否定断言；
//  ② 通道**两边都还在**：主进程已注册（handleIpc）+ preload 已暴露（ipcRenderer.invoke）。
const lanMeshRetiredUiIds = [
  'anNiuNeiWangQiDong', 'anNiuNeiWangTingZhi', 'anNiuNeiWangFaSong', 'anNiuNeiWangShuangJi',
  'neiWangDuanKou', 'neiWangHost', 'neiWangPport', 'neiWangXiaoXi', 'neiWangShouXiang',
  'anNiuWangZhuangQiDong', 'anNiuWangZhuangTingZhi', 'anNiuWangZhuangBcast',
  'wangZhuangDuanKou', 'duiDuanMing', 'duiDuanHost', 'duiDuanDuanKou', 'anNiuDuiDuanTianJia',
...
if (!appJs.includes('id="wangLuoKa"')) {
  lanMeshUiProblems.push('successor 组网设置 ka (id="wangLuoKa") not found — 旧块不该靠"删干净"过关');
}
check('renderer lan/mesh settings blocks removed', lanMeshUiProblems.length === 0, lanMeshUiProblems);

const lanMeshChannels = [
  'warmy:neiWangQiDong', 'warmy:neiWangTingZhi', 'warmy:neiWangFaSong', 'warmy:neiWangShouXiang', 'warmy:neiWangZhuangTai', 'warmy:neiWangShuangJiMaoYan',
  'warmy:wangZhuangQiDong', 'warmy:wangZhuangTingZhi', 'warmy:wangZhuangGuangBo', 'warmy:wangZhuangShouXiang', 'warmy:wangZhuangZhuangTai',
];
...
check('renderer webgpu test', appJs.includes('anNiuwebgpu'));

const { shuangJiMaoYan } = await import(
  toImportUrl(path.join(ascii, 'sync-protocol', 'dist', 'lan.js'))
);
const lan = await shuangJiMaoYan({ localId: 'verify-node', localPort: 7799 });
