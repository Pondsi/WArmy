...
const rep = [
  ...
  ["name: '项目推进群'", "name: 'demo.project1'"],
  ["name: '研发排期群'", "name: 'demo.project2'"],
  ["name: '客户对接群'", "name: 'demo.client'"],
  ["name: '主力牛马'", "name: 'demo.agent'"],
  ...
  ["(state.profile.username || '我').slice(0, 1)", "(state.profile.username || t('nav.avatar')).slice(0, 1)"],
...
// popup key usage
j = j.replace(/\$\('smtp-label'\)\.value\.trim\(\),/, "$('smtp-label').value.trim(),");
